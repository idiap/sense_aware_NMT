...
class Translator(object):
    def __init__(self, opt):
        ...
        self.feature_dict = checkpoint['dicts']['feature']

        encoder = onmt.Models.Encoder(model_opt, self.src_dict, self.feature_dict)
    ...
    def buildData(self, srcBatch, fea1_Batch, fea2_Batch, fea3_Batch, fea4_Batch, fea5_Batch, goldBatch):

        srcData = [self.src_dict.convertToIdx(b,
                    onmt.Constants.UNK_WORD) for b in srcBatch]
        fea1_Data = [self.feature_dict.convertToIdx(b,
                    onmt.Constants.UNK_WORD) for b in fea1_Batch]
        fea2_Data = [self.feature_dict.convertToIdx(b,
                    onmt.Constants.UNK_WORD) for b in fea2_Batch]
        fea3_Data = [self.feature_dict.convertToIdx(b, onmt.Constants.UNK_WORD) for b in fea3_Batch]
        fea4_Data = [self.feature_dict.convertToIdx(b, onmt.Constants.UNK_WORD) for b in fea4_Batch]
        fea5_Data = [self.feature_dict.convertToIdx(b, onmt.Constants.UNK_WORD) for b in fea5_Batch]
    ...
    def translateBatch(self, srcBatch, fea1_Batch, fea2_Batch, fea3_Batch, fea4_Batch, fea5_Batch, tgtBatch):
        batchSize = srcBatch[0].size(1)
        beamSize = self.opt.beam_size

        #  (1) run the encoder on the src
        encStates, context = self.model.encoder(srcBatch, fea1_Batch, fea2_Batch, fea3_Batch, fea4_Batch, fea5_Batch)
    ...
    def translate(self, srcBatch, fea1_Batch, fea2_Batch, fea3_Batch, fea4_Batch, fea5_Batch, goldBatch):


        #  (1) convert words to indexes

        dataset = self.buildData(srcBatch, fea1_Batch, fea2_Batch, fea3_Batch, fea4_Batch, fea5_Batch, goldBatch)
        src, feature_1, feature_2, feature_3, feature_4, feature_5, tgt, indices = dataset[0]

        #  (2) translate
        pred, predScore, attn, goldScore = self.translateBatch(src, feature_1, feature_2, feature_3, feature_4, feature_5, tgt)
